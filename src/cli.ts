...
async function interactiveCLI() {
  ...
  try {
    ...
    let chain: string;
    switch (chainChoice) {
      case "1": chain = "celo"; break;
      case "2": chain = "ethereum"; break;
      case "3": chain = "base"; break;
      default:
...
yargs(hideBin(process.argv))
  ...
  .command("push-quick", "Trigger STK push (command line)", (y) => {
    ...
    await triggerStkPushCLI(argv.phone as string, argv.amount as number, argv.chain as string, argv.token as string, argv.recipient as string);
  })
  .demandCommand(1)
  .help()
  .parse();
