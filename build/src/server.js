...
exports.triggerStkPushCLI = triggerStkPushCLI;
...
// RPC providers
const providers = {
    celo: new ethers_1.ethers.JsonRpcProvider(process.env.CELO_RPC),
    ethereum: new ethers_1.ethers.JsonRpcProvider(process.env.ETH_RPC),
    base: new ethers_1.ethers.JsonRpcProvider(process.env.BASE_RPC),
};
...
async function getMpesaToken() {
    ...
    const res = await axios_1.default.get(OAUTH_URL, { headers: { Authorization: auth } });
...
async function stkPush(phone, amountKES, accountRef, callbackUrl) {
    ...
    const resp = await axios_1.default.post(STK_URL, body, { headers: { Authorization: `Bearer ${token}` } });
...
/** CLI helper */
async function triggerStkPushCLI(phone, amount, chain, token, recipient) {
    const callbackUrl = `${process.env.MPESA_CALLBACK_BASE}/mpesa/callback`;
...
async function processSwapAndSend(chain, paidKES, targetToken, recipient) {
    ...
    const usdc = new ethers_1.ethers.Contract(usdcAddress, ERC20_ABI, signer);
    const router = new ethers_1.ethers.Contract(routerAddr, ROUTER_ABI, signer);
